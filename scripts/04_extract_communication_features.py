"""
Script to extract communication features for all users.

Outputs:
- data/processed/features/communication_features.parquet
"""
...
import pandas as pd
from features.communication import CommunicationExtractor
...
def main():
    ...
    print(f"Extracting communication features for {len(user_ids)} users...")
    print("=" * 60)

    # Initialize extractor
    extractor = CommunicationExtractor(
        call_dir=call_dir,
        sms_dir=sms_dir,
    ...
    # Save to parquet
    output_file = output_dir / 'communication_features.parquet'
    ...
    feature_cols = [c for c in features_df.columns if c not in ['uid', 'comm_valid_days']]
