"""
Script to extract GPS mobility features for all users.

Outputs:
- data/processed/features/gps_mobility_features.parquet
"""
...
import pandas as pd
from features.gps_mobility import GPSMobilityExtractor


def main():
    # Paths
    data_dir = project_root / 'data' / 'raw' / 'dataset' / 'sensing' / 'gps'
    ...
    print(f"Extracting GPS mobility features for {len(user_ids)} users...")
    print("=" * 60)

    # Initialize extractor
    extractor = GPSMobilityExtractor(
        data_dir=data_dir,
        timelines_file=timelines_file,
        accuracy_threshold=100.0,  # 100m accuracy threshold
    ...
    # Save to parquet
    output_file = output_dir / 'gps_mobility_features.parquet'
    ...
    feature_cols = [c for c in features_df.columns if c not in ['uid', 'gps_valid_days', 'gps_total_points', 'gps_points_per_day']]
