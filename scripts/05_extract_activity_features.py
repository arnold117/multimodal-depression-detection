"""
Script to extract activity features for all users.

Outputs:
- data/processed/features/activity_features.parquet
"""
...
def main():
    # Paths
    data_dir = project_root / 'data' / 'raw' / 'dataset' / 'sensing' / 'activity'
    ...
    print(f"Extracting activity features for {len(user_ids)} users...")
    print("=" * 60)

    # Initialize extractor with chunk-based processing
    ...
    # Save to parquet
    output_file = output_dir / 'activity_features.parquet'
    ...
    feature_cols = [c for c in features_df.columns if c not in ['uid', 'activity_valid_days', 'activity_total_samples']]
