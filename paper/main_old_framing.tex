...
% ══════════════════════════════════════════════════════════════════════
\begin{abstract}
Big Five personality traits---particularly Neuroticism---are well-established correlates of internalizing psychopathology, yet multi-study replications using machine learning methods remain scarce, and the clinical utility of personality-based prediction has rarely been evaluated against passive behavioral sensing. We addressed these gaps across three independent university samples (Dartmouth, Notre Dame, University of Washington; $N = 1{,}559$) using a unified machine learning pipeline (Ridge, Elastic Net, Random Forest, SVR with Optuna tuning and SHAP interpretation) to predict depression, anxiety, stress, and loneliness from Big Five traits and passively sensed smartphone and wearable data. Neuroticism emerged as the top-ranked predictor in all 28 SHAP models spanning seven mental health instruments, with meta-analytic correlations of $r = .63$ for anxiety, $r = .47$ for stress, and $r = .44$ for depression. Personality-only models achieved AUC = .65--.80 for clinical-cutoff classification and $R^2 = .09$--.52 for continuous prediction, whereas behavior-only models performed near chance (AUC = .53--.60; $R^2 \leq 0$ in 12/15 comparisons). Passive sensing added significant incremental validity in only 1 of 8 tests after FDR correction ($\Delta R^2 = .024$). A notable exception was Study~2, where personality-plus-behavior models reached AUC = .83--.86, driven by communication metadata. SHAP rankings were virtually identical to OLS regression coefficients ($\tau = .94$), and multilayer perceptrons underperformed Ridge regression, confirming that the personality--mental health relationship is linear and that simpler models suffice. These findings position brief personality assessment---as few as two Neuroticism items---as a robust, low-cost mental health screening tool that outperforms weeks of continuous passive monitoring.
\end{abstract}
...
Across three independent university samples totaling $N = 1{,}559$ participants, Big Five personality traits---and Neuroticism in particular---emerged as robust, replicable predictors of mental health outcomes. These findings converge across seven self-report instruments, four machine learning algorithms, and universities spanning different regions and time periods. We discuss the implications in five sections, addressing the centrality of Neuroticism, the limited added value of passive behavioral sensing, the sufficiency of simple models, the boundaries of our evidence, and directions for future research.

\subsection{Neuroticism as a Transdiagnostic Marker}

The most consistent finding across all analyses was the dominance of Neuroticism as a predictor of mental health. Neuroticism ranked as the most important feature in all 28 SHAP models estimated across the three studies, spanning depression (PHQ-9, CES-D, BDI-II), anxiety (STAI, BAI), stress (PSS-10), and loneliness (UCLA Loneliness Scale). Meta-analytic aggregation confirmed large pooled effects: $r = .63$ for anxiety, $r = .44$ for depression, and $r = .47$ for stress. These effect sizes align with \citet{kotov2010linking}, whose meta-analysis of 175 studies identified Neuroticism as the strongest personality correlate of internalizing disorders ($d = 1.65$ for depression, $d = 1.01$ for anxiety). Our contribution is not the direction of this association---which has been well-established since at least \citet{costa1987neuroticism}---but its consistency across a machine learning framework that makes no parametric assumptions about the form of the personality--outcome relationship.

The trait specificity of the findings strengthens the case for Neuroticism as a transdiagnostic vulnerability marker \citep{lahey2009public}. Conscientiousness emerged as the dominant predictor of academic performance (GPA) in all eight models where GPA was an outcome, whereas Neuroticism was the dominant predictor of every mental health outcome---a double dissociation that was never reversed across studies. This pattern is consistent with a theoretical framework in which Conscientiousness captures behavioral regulation relevant to academic effort and persistence, while Neuroticism captures emotional reactivity and negative affectivity that cut across diagnostic categories \citep{widiger2007plate}. That two items from the BFI---``Is depressed, blue'' and ``Is emotionally stable, not easily upset''---explained $R^2 = .36$ for CES-D depression scores underscores the clinical efficiency of Neuroticism: a 10-second self-report captured more between-person variance than weeks of continuous passive sensing ($R^2 = -.16$ for 28 behavioral features).

The robustness of these findings across measurement contexts is noteworthy. The Neuroticism--mental health association held whether personality was measured with the 44-item BFI \citep{john1999big} (Studies~1 and~2) or the abbreviated BFI-10 \citep{rammstedt2007measuring} (Study~3), despite the latter's reduced reliability ($\alpha \approx .65$). Controlling for gender, race, native-English-speaker status, parental education, and family income reduced personality's predictive contribution only modestly (personality $\Delta R^2 = .20$--.52 after demographic controls, versus demographics alone $R^2 = .03$--.06). Excluding the pandemic-era cohort (2020--2021) produced negligible changes ($\Delta R^2 < .015$), confirming that the association is not an artifact of COVID-related distress.

\subsection{The Limited Value of Passive Behavioral Sensing}

Despite substantial investment in digital phenotyping as a mental health monitoring tool \citep{insel2018digital, torous2021growing}, passive behavioral sensing contributed minimally to prediction in our data. Behavior-only models produced $R^2 \leq 0$ in 12 of 15 outcome comparisons---performing worse than predicting the sample mean. In clinical classification, behavior-only AUC ranged from .53 to .60, barely above chance, compared to personality-only AUC of .65 to .79. Formal incremental validity testing revealed that only 1 of 8 comparisons survived Benjamini--Hochberg FDR correction (Study~3 BDI-II, $\Delta R^2 = .024$, $p_{\mathrm{FDR}} = .047$)---a statistically significant but clinically negligible contribution.

The exception merits discussion. In Study~2, the combined personality-plus-behavior model achieved AUC = .83--.86 for depression and trait anxiety, a meaningful improvement over personality alone (AUC = .75--.79). This gain was driven by communication metadata---call frequency and SMS patterns from the NetHealth dataset---features that capture social interaction, which is more semantically proximal to mental health than step counts or sleep duration. Study~3, which lacked communication features, showed no comparable benefit. This pattern suggests that when sensing captures social behavior rather than mere physical activity, it can provide marginal incremental information, but the conditions for this contribution are restrictive.

We emphasize that this is not a technology failure per se. Rather, it reflects what \citet{dasswain2022semantic} termed a ``semantic gap'' between low-level sensor signals and the higher-order psychological constructs they are intended to index. Personality questionnaires directly probe stable dispositional tendencies that are theoretically linked to psychopathology; passive sensing captures moment-to-moment behavioral fluctuations influenced by weather, academic schedules, and social obligations in addition to any underlying trait signal. Our dose-response analysis confirmed that extending data collection from 7 to 92 days did not improve sensing performance, suggesting the between-person signal is not buried in short measurement windows but is fundamentally absent at the population level. Converging evidence from \citet{muller2021depression}, who found GPS-based depression prediction dropped from AUC = .82 to .57 in large samples, and from the GLOBEM benchmark \citep{xu2022globem}, which reported near-chance cross-cohort generalization, supports this interpretation.

\subsection{Simpler Models Suffice}

A methodologically important finding is that analytic complexity provided no benefit for the personality--mental health prediction task. SHAP-derived feature importance rankings were virtually identical to standardized OLS regression coefficients (mean Kendall's $\tau = .94$ across seven personality--outcome comparisons), indicating that penalized linear models captured the same structure that ordinary regression reveals. Multilayer perceptrons consistently underperformed Ridge regression, often catastrophically so in smaller samples (Study~1 MLP $R^2 = -4.06$ for Flourishing). Optuna hyperparameter optimization, applied systematically across all model types, did not uncover configurations that materially improved performance.

These results suggest that the personality--mental health relationship is predominantly linear: five broad trait dimensions predict internalizing symptoms through additive main effects, with no meaningful interactions or non-linear thresholds for machine learning to exploit. This finding has practical implications. SHAP analysis, while computationally expensive, served primarily a communication function---producing intuitive visualizations of feature importance---rather than revealing structure invisible to traditional regression. For personality-based mental health prediction, Ridge or Elastic Net regression with cross-validated regularization appears sufficient. The field's investment in increasingly complex architectures (deep learning, transformers, attention mechanisms) may be misaligned with the low-dimensional, linear nature of the personality--outcome signal \citep{roberts2007power}.

\subsection{Limitations}

Several limitations constrain the generalizability of our findings. First, all three samples comprise undergraduate students at selective U.S.\ universities (Dartmouth, Notre Dame, University of Washington), restricting inference to relatively affluent, young, and predominantly White populations. Whether personality's predictive dominance extends to clinical populations, older adults, or non-Western cultural contexts---where the structure of personality and its relation to psychopathology may differ---remains an open question.

Second, our design is cross-sectional with respect to the core prediction models. Although supplementary longitudinal and trajectory analyses confirmed that personality predicts mean symptom levels but not within-person change over time---consistent with a trait interpretation---we cannot make causal claims about the personality--mental health association. Prospective designs that assess personality before disorder onset would provide stronger evidence for the screening utility we propose.

Third, Study~3 used the BFI-10 \citep{rammstedt2007measuring}, which has lower internal consistency ($\alpha \approx .65$) than the BFI-44 used in Studies~1 and~2. Attenuation due to measurement error biases results \textit{against} personality, and our disattenuation analyses confirmed that reliability-corrected effects are larger than observed. Nevertheless, the use of an abbreviated instrument in the largest study introduces measurement imprecision.

Fourth, we analyzed secondary data not originally designed for this specific investigation. Sensing modalities differ across studies---Study~2 included Fitbit wearable and communication metadata, while Studies~1 and~3 relied primarily on smartphone sensing---and no single study captures all possible behavioral domains (e.g., GPS, social media, physiological signals). The absence of these modalities means we cannot rule out the possibility that unmeasured behavioral features carry stronger predictive signal.

Fifth, all mental health outcomes are self-report symptom scales, not clinical diagnoses. Questionnaire cutoffs (e.g., CES-D $\geq$ 16, BDI-II $\geq$ 14) approximate but do not equate to DSM-5 diagnostic criteria, and predictive performance in clinical diagnostic contexts may differ.
...
Our findings carry implications for both clinical practice and research methodology. First, personality screening represents a low-cost, scalable triage tool for identifying students at elevated risk for mental health difficulties. A two-item Neuroticism assessment, requiring approximately 10 seconds to complete, explained more variance in depression than weeks of continuous behavioral monitoring. In resource-constrained settings---university counseling centers, primary care waiting rooms, large-scale online surveys---personality screening could serve as a first-pass risk stratification tool, directing clinical resources toward individuals with the highest trait vulnerability \citep{lahey2009public}.

Second, we recommend that digital phenotyping studies routinely include a personality questionnaire baseline. Without this comparator, it is impossible to determine whether sensing provides unique information or merely recapitulates variance already accessible through a brief self-report. Our meta-analytic personality benchmarks ($R^2 = .09$--.53 across outcomes) provide reference values against which sensing contributions can be evaluated.

Third, future research should prioritize longitudinal prediction: does personality assessed at university entrance predict symptom trajectories across semesters, and can sensing add incremental value for detecting within-person deviations from trait-predicted baselines? Our idiographic analyses suggest that 17\% of individuals show meaningful person-specific sensing--mood associations, hinting that sensing's value may lie in personalized monitoring of high-risk individuals identified by initial trait assessment, rather than population-level screening.

% ══════════════════════════════════════════════════════════════════════
\section{Conclusion}

Across three universities and 1,559 participants, Big Five personality traits---led by Neuroticism---robustly predicted depression, anxiety, stress, and loneliness, replicating across seven instruments, four machine learning algorithms, and diverse demographic contexts. Neuroticism emerged as the top-ranked predictor in all 28 SHAP models, with meta-analytic correlations of $r = .44$--.63, while passive behavioral sensing added negligible incremental validity (1/8 comparisons significant after FDR correction). The personality--mental health relationship was predominantly linear, with Ridge regression matching or exceeding more complex models including multilayer perceptrons. These findings position brief personality assessment as a robust, low-cost mental health screening tool and suggest that the field's pursuit of complex sensing and modeling architectures may be less productive than leveraging the well-established, efficient, and replicable signal carried by a small number of personality trait items.
