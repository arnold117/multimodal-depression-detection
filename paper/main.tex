...
These findings suggest that a brief personality questionnaire could serve as a remarkably efficient baseline for mental health prediction. The Big Five Inventory \citep[BFI;][]{john1999big} captures five broad trait dimensions in 44 items (approximately five minutes), and even its ultra-short 10-item version \citep[BFI-10;][]{rammstedt2007measuring} retains adequate predictive validity. From a purely practical standpoint, the personality questionnaire is difficult to beat: it is free, requires no hardware, is language-agnostic across validated translations, introduces no privacy concerns beyond standard survey consent, and produces reliable individual-difference scores from a single administration. Clinical evidence further supports personality's diagnostic utility. \citet{ho2022diagnostic} demonstrated that the 10-item TIPI personality questionnaire distinguished MDD patients from healthy controls with excellent accuracy (AUC = 0.90) in a Singaporean clinical sample, with neuroticism increasing the odds of MDD 8.82-fold per standard deviation. Building on this, \citet{tay2026layered} proposed a layered vulnerabilities framework in which personality traits operate as distal risk factors, coping styles as semi-malleable mediators, and subjective cognitive complaints as proximal markers---notably, objective cognitive test performance showed no group differences, while subjective self-report measures were highly predictive. This pattern---objective measures failing where subjective self-report succeeds---foreshadows our central finding with passive sensing. Despite these advantages, personality questionnaires are often viewed as ``old school'' within the digital health community and have never been systematically pitted against passive behavioral sensing in a fair comparison.
...
This disparity is not, we argue, primarily a technology problem. It reflects a construct mismatch between what sensors measure and what mental health screening requires. Personality questionnaires directly probe stable dispositional tendencies---precisely the trait-level constructs (Neuroticism, emotional stability) that are theoretically and empirically linked to psychopathology \citep{kotov2010linking, lahey2009public}. Passive sensing, by contrast, captures moment-to-moment behavioral manifestations (step counts, screen unlocks, sleep duration) that are influenced by a host of situational factors---weather, academic schedules, social obligations---in addition to any underlying trait signal. \citet{dasswain2022semantic} formalized this intuition as the ``semantic gap'' between low-level sensor signals and the higher-order psychological constructs they are intended to index, arguing that increasing data volume cannot close a fundamentally inferential chasm. Our results across 1,559 participants and 15 outcome comparisons provide strong empirical support for this theoretical account. Even \citet{stachl2020predicting}, using rich smartphone data from 743 participants to predict personality itself, achieved a maximum correlation of $r = .37$---and predicting mental health from behavior requires traversing an additional inferential step from behavioral proxies to personality to psychopathology. This finding converges with clinical evidence from \citet{tay2026layered}, who found that objective cognitive tests failed to distinguish MDD patients from healthy controls despite large differences in subjective cognitive complaints ($d = 1.78$). Across both domains---cognitive testing and passive sensing---objective, performance-based measures consistently underperform subjective self-report, suggesting a broader principle: self-report captures internal psychological states that objective behavioral proxies cannot access.
...
% ══════════════════════════════════════════════════════════════════════
% TODO: Switch to AMA 11th Edition style for JMIR Mental Health submission
% JMIR requires: structured abstract ≤450 words, main body ≤10,000 words
% Supplementary analyses → Multimedia Appendices
